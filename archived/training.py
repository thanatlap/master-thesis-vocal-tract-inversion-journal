...
def main(args):
	...
	if args.exp == 120: ptraining_fn(nn.init_baseline(), model_name='baseline')
	if args.exp == 121: ptraining_fn(nn.init_FCNN(), model_name='FCNN')
	if args.exp == 122: ptraining_fn(nn.init_bilstm(), model_name='bilstm')
	if args.exp == 123: ptraining_fn(nn.init_LTRCNN(), model_name='LTRCNN')
	if args.exp == 124: ptraining_fn(nn.init_senet(feature_layer=1,
										bilstm = 3, 
										se_enable = True, 
										cnn_unit=128, 
										bilstm_unit=256, 
	...
	if args.exp in range(125,130):
		cf.DATASET_DIR = '../data/d_dataset_p2/prep_data_13'
		X_train, X_val, X_test, y_train, y_val, y_test = prep_data()
		ptraining_fn = partial(training_fn, 
			X_train=X_train, 
			X_val=X_val, 
	...
	if args.exp == 125: ptraining_fn(nn.init_baseline(), model_name='baseline')
	if args.exp == 126: ptraining_fn(nn.init_FCNN(), model_name='FCNN')
	if args.exp == 127: ptraining_fn(nn.init_bilstm(), model_name='bilstm')
	if args.exp == 128: ptraining_fn(nn.init_LTRCNN(), model_name='LTRCNN')
	if args.exp == 129: ptraining_fn(nn.init_senet(feature_layer=1,
										bilstm = 3, 
										se_enable = True, 
										cnn_unit=128, 
										bilstm_unit=256, 
	...
	# rerun, reducing bilistm unit to 128
	if args.exp == 134: 
		cf.DATASET_DIR = '../data/m_dataset_p2/prep_data_13'
		X_train, X_val, X_test, y_train, y_val, y_test = prep_data()
		training_fn(nn.init_senet(feature_layer=1,
	...
	if args.exp == 149: 
		cf.DATASET_DIR = '../data/d_dataset_p2/prep_data_13_noaug'
		X_train, X_val, X_test, y_train, y_val, y_test = prep_data()
		training_fn(nn.init_senet(feature_layer=1,
										bilstm = 3, 
										se_enable = True, 
	...
	if args.exp == 150: 
		cf.DATASET_DIR = '../data/d_dataset_p2/prep_data_13_v2'
		X_train, X_val, X_test, y_train, y_val, y_test = prep_data()
		training_fn(nn.init_senet(feature_layer=1,
										bilstm = 3, 
										se_enable = True, 
	...
	if args.exp == 151: 
		cf.DATASET_DIR = '../data/d_dataset_3/prep_data_13_v2'
		X_train, X_val, X_test, y_train, y_val, y_test = prep_data()
		training_fn(nn.init_senet(feature_layer=1,
										bilstm = 3, 
										se_enable = True, 
