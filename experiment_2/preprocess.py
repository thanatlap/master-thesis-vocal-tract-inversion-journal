...
def segment_audio(audio_data, mode, is_disyllable):
	'''
	segment data into an equal length
	The segment length is kept separately for D-AAI and M-AAI
	Since the lenght of the audio is equal (from VTL and manually preprocess) we can segment
	by trim 1000 datapoint for M-AAI and start 1000 and end 1000 for D-AAI 
	'''
	length_file = join('vars','seg_length.npy')
	# for training set, the length is save
	if mode in ['training']:
		os.makedirs('vars', exist_ok = True)

		seg_audio = np.array([ data[1000:-1000] if is_disyllable else data[1000:] for data in audio_data ])
		np.save(arr=seg_audio.shape[1], file=length_file)
	else:
		if not os.path.exists(length_file):
			raise ValueError('segment length not found')
		seg_length = np.load(length_file)
		seg_audio = np.array([ data[-seg_length:] for data in audio_data ]) 
	return seg_audio
...
def strech_audio(audio_data):
	...
	strech = [ librosa.effects.time_stretch(data, rate=np.random.uniform(low=0.8,high=1.5)) for data in audio_data]
	# if the strech data lenght is more more than original lenght, the strech lenght is cut. 	
	# for strech data that lenght less than original, the data is padding 
	return np.array([data[:input_length] if data.shape[0] > input_length else np.pad(data, (0, max(0, input_length - data.shape[0])), "constant") for data in audio_data])
...
def split_audio(audio_data, labels, mode):
	...
	'''
	# compute the length and split point
	audio_length = audio_data.shape[1]
	split_point_1 = math.ceil(0.5*audio_length)
	split_point_2 = math.floor(0.5*audio_length)
	# split audio
	split_audio = np.array([data[:split_point_1] if j == 0 else data[split_point_2:] for i, data in enumerate(audio_data) for j in range(2)])
...
def transfrom_mfcc(audio_data, sample_rate):
	...
	'''
	def wav2mfcc(data):
		mfcc = librosa.feature.mfcc(data, sr=sample_rate, n_mfcc = 13)[:13] # experiment with first mfcc features
		return mfcc
...
def normalized_mfcc(features, is_train):
...
def preprocess_pipeline(features, labels, mode, is_disyllable, sample_rate, is_train, is_cnn):
	if is_disyllable:
	...
	features = normalized_mfcc(features, is_train = is_train)
	print('[INFO] Completed')
	# swap dimension to (data, timestamp, features)
	print('[INFO] Swap axis to (data, timestamp, features) for RNN')
	features = np.swapaxes(features ,1,2)
	if is_cnn:
		# prepare for CNN 2d
		print('[INFO] Prepare for CNN 2d')
...
def main(args):
	...
	print('[INFO] Completed\n[INFO] Loading audio and labels data')
	...
	if args.mode != 'predict':
		...
		print('[INFO] Completed')
	# segment audio data into equal length
	print('[INFO] Segmenting audio data')
	audio_data = segment_audio(audio_data, mode=args.mode, is_disyllable=disyllable)
	print('[INFO] Completed')
	...
	if args.output_path == None:
		print('default output directory is used')
		args.output_path = 'prep_data'

	#used output path with the same directory as data path
	# ex, d_dataset_1/prep_data_npy_cnn/training_subsets.npz
	output_path = join(args.data_path, args.output_path+'_cnn') if args.cnn == True else join(args.data_path, args.output_path+'_rnn')
	# create output file
...
if __name__ == '__main__':
	...
	parser.add_argument('--rnn', dest='cnn', help='preprocess for RNN model', action='store_false')
	parser.add_argument('--cnn', dest='cnn', help='preprocess for CNN model', action='store_true')
